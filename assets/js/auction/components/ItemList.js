...
const ItemList = (() => {
    ...
    function init() {
        ...
        // 자동완성 선택 이벤트
        document.addEventListener('autocompleteSelected', (e) => {
    ...
    function getCurrentPageIndices() {
        ...
        const endIndex = Math.min(startIndex + paginationState.itemsPerPage, state.filteredResults.length);
    ...
    function applyLocalFiltering() {
        ...
        setTimeout(() => {
            try {
                ...
                state.filteredResults = newFilteredResults;
                renderItemsTable();
            } catch (error) {
