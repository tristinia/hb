...
class FilterUI {
    ...
    addFilterPanel(filter) {
        ...
        if (isMobile) {
            ...
            // 필터 타입에 따른 UI 요소 생성 (데스크톱용)
            switch (filter.type) {
                case 'range':
                    filterContent.innerHTML = this.createRangeFilterUI(filter);
                    break;
                case 'selection':
                case 'select':
                    filterContent.innerHTML = this.createSelectionFilterUI(filter);
                    break;
                case 'enchant':
                    filterContent.innerHTML = this.createEnchantFilterUI(filter);
                    break;
                case 'reforge-option':
                    filterContent.innerHTML = this.createReforgeOptionFilterUI(filter);
                    break;
                case 'reforge-status':
                    filterContent.innerHTML = this.createReforgeStatusFilterUI(filter);
                    break;
                case 'erg':
                    filterContent.innerHTML = this.createErgFilterUI(filter);
                    break;
                case 'special-mod':
                    filterContent.innerHTML = this.createSpecialModFilterUI(filter);
                    break;
                case 'set-effect':
                    filterContent.innerHTML = this.createSetEffectFilterUI(filter);
                    break;
                default:
                    filterContent.innerHTML = '<p>지원되지 않는 필터 유형입니다.</p>';
            }
