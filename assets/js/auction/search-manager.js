...
const SearchManager = (() => {
    ...
    function handleAutocompleteSelect(item, index) {
        ...
        state.searchTerm = typeof item === 'object' ? item.name : item;
        
        // 특별 카테고리 확인
        const specialCategories = ['인챈트 스크롤', '도면', '옷본'];
        const isSpecialCategory = item.subCategory && specialCategories.includes(item.subCategory);
